...
**2026-09-07 작업.** 저장소 예산을 고정값에서 **조절되는 값**으로 바꿨다. `prune` 이 매번
`pg_database_size()` 를 재고 사용률(70%/85%)에 따라 보관 기간을 좁혀 `prune_timeseries()` 에
인자로 넘긴다. 예산·평상시 기간은 환경변수(`DB_SIZE_BUDGET_MB`·`RETENTION_*`)라 요금제를
올리거나 커버리지를 늘려도 **마이그레이션이 필요 없다**. `/status` 가 현재 사용량과 실제
적용 중인 기간을 그대로 보여 주고, 90% 를 넘으면 `watchdog` 이 던진다.
일 롤업은 어느 단계에서도 줄이지 않고 원시는 3일 하한을 지킨다(그 밑은 §5-1 과 같은 사고다).
**db/functions.sql 은 주석만 바뀌었다 — 함수 정의가 동일해 재적용이 필요 없다.**

**멀티플랫폼 1단계를 얹었다(2026-09-07).** Wikidata 의 `P1733` 역방향 조회로 Steam appid 를
Wikidata 항목에 잇고, Xbox·PlayStation·Switch 출시 여부와 날짜를 게임 상세에 적는다.
실측 — **205개 중 182개 확정(89%) · 모호 3 · 콘솔 출시 235건(그중 날짜까지 87건)**.
...
| 단위 테스트 | 91개 통과 (`npm test`) — `db/*.sql` 을 실제로 실행하는 4개 포함 |
| 롤업의 정확성 | **검증됨 (2026-09-06 3차)** — 창을 버킷 경계로 스냅하고 7일치를 되돌렸다. 불일치 버킷 0개. §3-3 |
...
**증상.** 겉으로는 아무 문제가 없었다. 잡은 `ok` 로 끝나고, 테스트 87개가 전부 통과하고,
`watchdog` 도 조용했다. 그런데 `player_hourly` 의 버킷 1,700개 중 1,698개가
**표본 1~3개**만 담고 있었다(10분 수집이면 6개여야 한다).
...
이 잡은 매시 **37분**에 돈다. 창은 `(H-3):37 → H:37` 이라 가장 오래된 버킷 `H-3` 에는
`:43`·`:53` 두 개만 들어온다. 그런데 `ON CONFLICT DO UPDATE` 가 **한 시간 전에 6표본으로
온전히 계산해 둔 값을 그 두 개짜리 조각으로 덮는다.** 다음 실행 때는 이미 창 밖이라
영영 복구되지 않는다. appid 730 으로 대조한 실측:

| 버킷 | 저장된 표본 | 실제 원시 행 | 저장된 평균 | 진짜 평균 |
| --- | --- | --- | --- | --- |
| 09-05 22:00 | 2 | 7 | 783,087 | **842,319** |
| 09-05 23:00 | 2 | 6 | 654,983 | **691,789** |

`rollup_player_daily` 도 같은 구조라 **이틀 뒤부터** 같은 일이 났을 것이다. 그쪽이 더 나쁘다 —
일 롤업은 `prune` 이 지우지 않는 영구 보관 테이블이다.

**왜 아무도 몰랐나.** 테스트 87개가 전부 JS 였고 `db/*.sql` 은 한 줄도 덮지 않았다.
롤업은 홈의 변화율·목록 스파크라인·급상승·주간 차트·게임 상세 그래프가 전부 읽는 값이라,
여기가 틀리면 화면의 숫자가 전부 조금씩 틀리는데 **아무 데서도 실패하지 않는다.**

**한 것.** 창을 버킷 경계로 스냅했다(시간은 `date_trunc('hour', ...)`, 일은 KST 자정).
그리고 `tests/rollup.test.mjs` 를 만들었다 — PGlite(내장 Postgres)가 `db/schema.sql` 과
`db/functions.sql` 을 **배포와 같은 파일 그대로** 올려 실제로 롤업을 돌리고, 불변식 하나를 검사한다:
**롤업이 쓴 버킷은 그 버킷의 원시 스냅샷 전부를 요약한 값이어야 한다.**
고치기 전 이 테스트는 두 함수 모두에서 실패한다(확인함).

**적용까지 마쳤다.** 이 PC 에는 `psql` 도 `DATABASE_URL_DIRECT` 도 없어서 `npm run db:apply` 를
새로 만들었다(`scripts/db-apply.mjs` — 드라이버로 `db/*.sql` 을 문장 단위로 보낸다).
적용 뒤 `rollup_player_hourly(200)` · `rollup_player_daily(8)` 로 원시가 남은 7일치를 다시 집계했고,
시간 2,485행·일 302행이 갱신되며 **불일치 버킷이 0개**가 됐다. 위 표의 09-05 22:00 은 842,319 로 돌아왔다.
원시 보관이 7일이라 **그 이전 시간 버킷은 복구되지 않는다** — 잘린 값 그대로 남아 있다.
