...
import { createGamePassService, diffCatalog, SIGLS } from './gamepass.mjs';
...
export function createCollector({
  ...
  gamepass = createGamePassService(),
  fetcher = fetch,
  concurrency = 4,
  timeoutMs = 15_000,
  now = Date.now,
  ...
  // ---------------------------------------------------------------------------
  // 2-2. Game Pass — 입점·퇴점을 우리가 직접 비교해서 만든다.
  //
  //   마이크로소프트는 '지금 목록'만 공개한다. "어제와 무엇이 달라졌나"는 어디에도 없어서,
  //   매일 목록을 찍어 두고 우리가 비교한다. 이 사이트가 동접에 하는 일과 같다.
  //
  //   미문서 엔드포인트라 규율이 하나 더 있다 — **핵심 의존성으로 두지 않는다.**
  //   여기서 실패해도 던지는 것은 이 잡뿐이고, 나머지 수집과 화면은 그대로다.
  // ---------------------------------------------------------------------------
  async function collectGamePass() {
    const today = new Date().toISOString().slice(0, 10);

    const [consoleIds, pcIds, leavingIds] = await Promise.all([
      gamepass.collection(SIGLS.console),
      gamepass.collection(SIGLS.pc),
      gamepass.collection(SIGLS.leaving)
    ]);
    const onConsole = new Set(consoleIds);
    const onPc = new Set(pcIds);
    const leaving = new Set(leavingIds);
    const ids = [...new Set([...consoleIds, ...pcIds])];
    // 목록이 비었으면 카탈로그가 사라진 게 아니라 우리 요청이 막힌 것이다.
    // 이때 비교하면 1,000개가 전부 '퇴점'으로 기록된다 — 절대 안 된다.
    if (!ids.length) return { processed: 0, failed: 1, skipped: 'empty-catalog' };

    const products = await gamepass.products(ids, { today });
    if (products.length < ids.length / 2) {
      return { processed: 0, failed: 1, skipped: 'partial-catalog', got: products.length, want: ids.length };
    }

    // 기준선 판정. 처음 도는 것이면 입점·퇴점을 만들지 않는다 —
    // 처음 본 1,000개를 '오늘 입점'이라고 적으면 기록이 아니라 거짓말이다.
    const [{ known }] = await sql`SELECT COUNT(*)::int AS known FROM gamepass_catalog`;
    const previous = (await sql`SELECT product_id FROM gamepass_catalog WHERE removed_on IS NULL`)
      .map(row => row.product_id);
    const { added, removed, baseline } = diffCatalog({ previous, current: products, baseline: known === 0 });
    ...
    const rows = products.map(product => ({
      ...
      on_console: onConsole.has(product.productId),
      on_pc: onPc.has(product.productId),
      leaving_soon: leaving.has(product.productId)
    }));

    await sql`
      INSERT INTO gamepass_catalog
        (product_id, title, developer, image_url, release_date, on_console, on_pc, leaving_soon, first_seen_on, last_seen_on)
      SELECT x.product_id, x.title, x.developer, x.image_url, x.release_date,
             x.on_console, x.on_pc, x.leaving_soon, ${today}::date, ${today}::date
        FROM jsonb_to_recordset(${rowsAsJson(rows)}::jsonb)
          AS x(product_id text, title text, developer text, image_url text, release_date date,
               on_console boolean, on_pc boolean, leaving_soon boolean)
          ON CONFLICT (product_id) DO UPDATE
         SET title = EXCLUDED.title,
             developer = COALESCE(EXCLUDED.developer, gamepass_catalog.developer),
             image_url = COALESCE(EXCLUDED.image_url, gamepass_catalog.image_url),
             release_date = EXCLUDED.release_date,
             on_console = EXCLUDED.on_console,
             on_pc = EXCLUDED.on_pc,
             leaving_soon = EXCLUDED.leaving_soon,
             last_seen_on = ${today}::date,
             -- 돌아온 게임은 퇴점 표시를 지운다. first_seen_on 은 그대로 둔다 —
             -- 처음 본 날은 처음 본 날이고, 재입점은 events 에 남는다.
             removed_on = NULL`;

    if (removed.length) {
      await sql`UPDATE gamepass_catalog SET removed_on = ${today}::date
                 WHERE product_id = ANY(${removed}::text[]) AND removed_on IS NULL`;
    }

    // 사건 기록. 제목을 함께 박아 둔다 — 퇴점한 게임은 카탈로그에서 사라져 이름을 다시 얻을 수 없다.
    const events = [
      ...added.map(id => ({ product_id: id, event: 'added', title: rows.find(row => row.product_id === id)?.title ?? id })),
      ...removed.map(id => ({ product_id: id, event: 'removed', title: null }))
    ];
    if (events.length) {
      await sql`
        INSERT INTO gamepass_events (product_id, event, happened_on, title)
        SELECT x.product_id, x.event, ${today}::date,
               COALESCE(x.title, (SELECT c.title FROM gamepass_catalog c WHERE c.product_id = x.product_id), x.product_id)
          FROM jsonb_to_recordset(${rowsAsJson(events)}::jsonb) AS x(product_id text, event text, title text)
            ON CONFLICT (product_id, event, happened_on) DO NOTHING`;
    }

    // 입점 예정은 공식 발표에서만 나온다. 실패해도 여기서 멈추지 않는다 —
    // 예정을 못 읽었다고 오늘의 카탈로그 기록까지 버릴 이유가 없다.
    const announced = (await gamepass.announcements())
      .filter(item => item.announcedFor && item.announcedFor >= today);
    if (announced.length) {
      await sql`
        INSERT INTO gamepass_upcoming (title, announced_for, devices, source_url, announced_at)
        SELECT x.title, x.announced_for, x.devices, x.source_url, x.announced_at
          FROM jsonb_to_recordset(${rowsAsJson(announced.map(item => ({
    title: item.title, announced_for: item.announcedFor, devices: item.devices,
    source_url: item.sourceUrl, announced_at: item.postedAt
  })))}::jsonb) AS x(title text, announced_for date, devices text, source_url text, announced_at timestamptz)
            ON CONFLICT (title, announced_for) DO UPDATE
           SET devices = EXCLUDED.devices, source_url = EXCLUDED.source_url, observed_at = NOW()`;
    }
