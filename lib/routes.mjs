...
export const ROUTES = [
  ...
  // 콘솔 출시. 키는 세 개뿐이라 패턴에서 바로 가둔다 — 아무 문자열이나 받으면
  // 크롤러가 /platform/xxx 를 무한히 만들어 내고 그만큼 404 가 색인된다.
  {
    name: 'platform',
    source: '/platform/:key',
    pattern: /^\/platform\/(xbox|playstation|switch)$/,
    params: match => ({ key: match[1] }),
    query: ['key']
  },
  { name: 'platforms', source: '/platform', pattern: /^\/platform$/, params: () => ({}), query: [] },
  { name: 'gamepass', source: '/gamepass', pattern: /^\/gamepass$/, params: () => ({}), query: [] },
  ...
  // 계정. **자율 기능이다** — 이 세 주소 말고는 어디에도 로그인이 필요하지 않다.
  // 셋 다 사람마다 다른 화면이라 no-store + noindex 다(pages.mjs 가 붙인다).
  { name: 'accountLogin', source: '/account/login', pattern: /^\/account\/login$/, params: () => ({}), query: [] },
  { name: 'accountSignup', source: '/account/signup', pattern: /^\/account\/signup$/, params: () => ({}), query: [] },
  { name: 'account', source: '/account', pattern: /^\/account$/, params: () => ({}), query: [] },
