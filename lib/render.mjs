...
export const COL = {
  ...
  review: (label = 'Steam 평가') => ({
    label, headClass: 'numeric', cellClass: 'numeric', cell: row => reviewCell(row)
  }),
  meta: (label = '메타크리틱') => ({
    label, headClass: 'numeric', cellClass: 'numeric', cell: row => metaScoreCell(row.metacritic_score, null)
  }),
  // 7일 추이. 표본이 얇으면 칸을 비우지 않고 '표본 부족'이라고 적는다 —
  // 빈 칸은 "추세가 없다"로 읽히지만 사실은 "아직 모른다"이다.
  spark: (label = '7일 추이') => ({
    label,
    headClass: 'spark-column',
    cellClass: 'spark-cell',
    cell: row => sparkline(row.spark, { label: `${row.title} 최근 7일 동시접속자 추이` })
      || '<span class="missing">표본 부족</span>'
  }),
  price: (label = '가격') => ({
    label,
...
// 목록 한 행에 들어가는 미니 차트. 축도 눈금도 없고 모양만 있다.
//
...
export function sparkline(values, { label = '' } = {}) {
  ...
  const title = label || `표본 ${clean.length}일 · 최저 ${num(min)} · 최고 ${num(max)}`;

  return `<svg class="spark ${rising ? 'up' : 'down'}" viewBox="0 0 100 100" preserveAspectRatio="none" ` +
    `role="img" aria-label="${esc(title)}"><path d="${line}"/></svg>`;
}
