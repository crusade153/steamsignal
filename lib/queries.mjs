...
export function dailySeries(sql, appids, days = 7) {
  return sql`
    SELECT appid, ARRAY_AGG(avg_players ORDER BY day) AS points
      FROM player_daily
     WHERE appid = ANY(${appids}::int[])
       AND day >= (NOW() AT TIME ZONE 'Asia/Seoul')::date - ${clampLimit(days, 7, 90)}::int
     GROUP BY appid`;
}

export function hourlySeries(sql, appids, hours = 48) {
  return sql`
    SELECT appid, ARRAY_AGG(avg_players ORDER BY bucket) AS points
      FROM player_hourly
     WHERE appid = ANY(${appids}::int[])
...
// 스파크라인 재료를 고른다. 일 롤업이 우선이고, 적재 초기처럼 그게 얇으면 시간 롤업으로 내려간다
// — 게임 상세의 큰 차트와 같은 규칙이다.
//
// **내려갈 때는 이름도 같이 바꾼다.** 48시간을 '7일 추이'라고 부르면 거짓말이 되고,
...
export async function sparkSeries(sql, appids, { days = 7, hours = 48 } = {}) {
  const empty = { points: new Map(), label: `${days}일 추이` };
  ...
  if (usable >= appids.length * 0.8) {
    return { points: new Map(daily.map(row => [row.appid, row.points])), label: `${days}일 추이` };
  }

  const hourlyRows = await hourlySeries(sql, appids, hours);
  return { points: new Map(hourlyRows.map(row => [row.appid, row.points])), label: `${hours}시간 추이` };
}
