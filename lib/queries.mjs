...
// 저장소 사용량. /status 가 보여 주고 prune 이 보관 기간을 정할 때 함께 읽는다.
//
// 용량을 못 재면 예산을 지킬 수 없다 — 한도를 넘기고 나서야 아는 사고가
// Neon 무료 요금제에서는 '쓰기가 막히는' 형태로 온다.
// 논리 크기(pg_database_size)는 DELETE 직후에 줄지 않는다. 죽은 행이 자리를 비켜 주는 것은
// autovacuum 이 지난 뒤이고, 그 자리는 다음 INSERT 가 재사용한다. 그래서 prune 의 목적은
// '지금 줄이는 것'이 아니라 **증가를 멈추는 것**이다.
export function storageUsage(sql) {
  return sql`
    SELECT pg_database_size(current_database())::bigint AS total_bytes,
           (SELECT COALESCE(JSON_AGG(t ORDER BY t.bytes DESC), '[]'::json) FROM (
...
// 이 게임이 다른 기계에 언제 나왔나. DATE 는 규칙대로 문자열로 꺼낸다.
//
// released_on 이 NULL 인 행을 버리지 않는다. '플랫폼은 확실한데 날짜 미상'은
// 결측이지 부재가 아니고, 화면에서도 그렇게 구분해서 쓴다.
export function platformReleases(sql, appid) {
  return sql`
    SELECT platform, TO_CHAR(released_on, 'YYYY-MM-DD') AS released_on, source, observed_at
      FROM platform_releases
     WHERE appid = ${Number(appid)}
     ORDER BY released_on NULLS LAST, platform`;
}

// 목록 표의 플랫폼 배지. 행마다 질의하지 않고 appid 묶음으로 한 번에 받는다
// (sparkSeries 와 같은 방식이다 — 목록 하나에 질의 하나).
export async function platformsForApps(sql, appids) {
  const ids = [...new Set(appids.map(Number).filter(Number.isSafeInteger))];
  if (!ids.length) return new Map();
  const rows = await sql`
    SELECT appid, platform, TO_CHAR(released_on, 'YYYY-MM-DD') AS released_on
      FROM platform_releases
     WHERE appid = ANY(${ids}::int[])
     ORDER BY appid, platform`;
  const byApp = new Map();
  for (const row of rows) {
    if (!byApp.has(row.appid)) byApp.set(row.appid, []);
    byApp.get(row.appid).push({ platform: row.platform, released_on: row.released_on });
  }
  return byApp;
}

// 한 플랫폼에 나온 게임 목록. 동접 순으로 정렬하되 **동접이 없다고 빼지 않는다** —
// 차트 밖 게임도 그 기계에 나온 것은 사실이다.
export function gamesByPlatform(sql, platform, limit = 100) {
  return sql`
    SELECT a.appid, a.title, a.slug, a.header_image, a.genres, a.metacritic_score, a.metacritic_url,
           s.players, s.peak_today, s.rank,
           s.final_price, s.initial_price, s.discount_percent, s.price_formatted,
           s.positive_ratio, s.total_positive, s.total_negative, s.review_desc,
           TO_CHAR(r.released_on, 'YYYY-MM-DD') AS platform_released_on
      FROM platform_releases r
      JOIN apps a USING (appid)
      LEFT JOIN app_stats s USING (appid)
     WHERE r.platform = ${String(platform)}
     ORDER BY s.players DESC NULLS LAST, a.title
     LIMIT ${clampLimit(limit, 100, 200)}`;
}

// 플랫폼별 게임 수. 목록 링크와 사이트맵이 이 숫자로 '열리는 페이지'를 판정한다.
export function platformCounts(sql) {
  return sql`
    SELECT platform, COUNT(*)::int AS games,
           COUNT(released_on)::int AS dated,
           COUNT(*) FILTER (WHERE released_on > (NOW() AT TIME ZONE 'Asia/Seoul')::date)::int AS upcoming
      FROM platform_releases
     GROUP BY platform
     ORDER BY games DESC`;
}

// 매칭 상태. 아직 조회하지 않은 것과 조회했는데 없는 것을 구분해서 센다 —
// /status 가 이 층위의 커버리지를 감추지 않고 그대로 적는다.
export function platformCoverage(sql) {
  return sql`
    SELECT (SELECT COUNT(*)::int FROM apps) AS tracked,
           (SELECT COUNT(*)::int FROM game_sources WHERE match_status = 'matched') AS matched,
           (SELECT COUNT(*)::int FROM game_sources WHERE match_status = 'unmatched') AS unmatched,
           (SELECT COUNT(*)::int FROM game_sources WHERE match_status = 'ambiguous') AS ambiguous,
           (SELECT COUNT(*)::int FROM game_sources) AS checked,
           (SELECT COUNT(*)::int FROM platform_releases) AS releases,
           (SELECT COUNT(*)::int FROM platform_releases WHERE released_on IS NOT NULL) AS dated,
           (SELECT MAX(checked_at) FROM game_sources) AS last_checked`;
}

// --- Game Pass --------------------------------------------------------------

// 지금 카탈로그에 있는 것 / 곧 나가는 것 / 최근에 오간 것.
// 사건(입점·퇴점)은 우리가 매일 비교해서 만든 기록이라 남이 주는 값이 아니다.
export function gamepassSummary(sql) {
  return sql`
    SELECT (SELECT COUNT(*)::int FROM gamepass_catalog WHERE removed_on IS NULL) AS present,
           (SELECT COUNT(*)::int FROM gamepass_catalog WHERE removed_on IS NULL AND on_console) AS console_games,
           (SELECT COUNT(*)::int FROM gamepass_catalog WHERE removed_on IS NULL AND on_pc) AS pc_games,
           (SELECT COUNT(*)::int FROM gamepass_catalog WHERE removed_on IS NULL AND leaving_soon) AS leaving,
           (SELECT COUNT(*)::int FROM gamepass_catalog WHERE removed_on IS NOT NULL) AS gone,
           (SELECT COUNT(*)::int FROM gamepass_events) AS events,
           (SELECT TO_CHAR(MIN(first_seen_on), 'YYYY-MM-DD') FROM gamepass_catalog) AS watching_since,
           (SELECT MAX(last_seen_on)::text FROM gamepass_catalog) AS last_seen`;
}

// 곧 종료. 공식 '곧 종료' 컬렉션이 준 목록이라 우리가 판단한 값이 아니다.
export function gamepassLeaving(sql, limit = 40) {
  return sql`
    SELECT product_id, title, developer, image_url, on_console, on_pc,
           TO_CHAR(first_seen_on, 'YYYY-MM-DD') AS first_seen_on
      FROM gamepass_catalog
     WHERE removed_on IS NULL AND leaving_soon
     ORDER BY title
     LIMIT ${clampLimit(limit, 40, 100)}`;
}

// 입점 예정. 공식 발표 글에서 읽은 값이라 출처를 함께 꺼낸다.
export function gamepassUpcoming(sql, limit = 40) {
  return sql`
    SELECT title, TO_CHAR(announced_for, 'YYYY-MM-DD') AS announced_for, devices, source_url
      FROM gamepass_upcoming
     WHERE announced_for >= (NOW() AT TIME ZONE 'Asia/Seoul')::date
     ORDER BY announced_for, title
     LIMIT ${clampLimit(limit, 40, 100)}`;
}

// 최근 입점·퇴점. 우리가 비교해서 만든 기록이다.
//
// 조건을 파라미터 하나로 표현한다. 이 드라이버는 태그드 템플릿 조각을 합성하지 못한다.
// (설명을 SQL 주석으로 넣지 않는 이유 — 템플릿 문자열 안의 백틱은 거기서 문자열을 끊는다.)
export function gamepassEvents(sql, { event = null, limit = 30 } = {}) {
  return sql`
    SELECT e.product_id, e.event, TO_CHAR(e.happened_on, 'YYYY-MM-DD') AS happened_on, e.title,
           c.developer, c.image_url, c.on_console, c.on_pc
      FROM gamepass_events e
      LEFT JOIN gamepass_catalog c USING (product_id)
     WHERE (${event}::text IS NULL OR e.event = ${event})
     ORDER BY e.happened_on DESC, e.title
