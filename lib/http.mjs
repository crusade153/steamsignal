...
import { chartTop, appById, changeContext, sparkSeries, platformsForApps } from './queries.mjs';
...
export function serializeGame(row) {
  return {
    ...
    // 콘솔 출시. 빈 배열은 'PC 전용', null 은 '아직 확인하지 않음'이다 — 둘을 섞지 않는다.
    platforms: Array.isArray(row.platforms) ? row.platforms.map(item => item.platform) : null
  };
}
...
export async function handleApi(req, res) {
  ...
  try {
    ...
    if (url.pathname === '/api/health') {
      ...
      // sparkLabel 을 함께 보내는 이유는 sparkSeries 주석에 있다 — 추적군 비중, 전일 동시간,
      // 일평균 전일 대비는 서로 다른 질문이라 실제 계산 기준을 화면에 밝혀야 한다.
      // 콘솔 출시도 같은 이유로 따로 묻는다. 실패해도 목록은 그대로 나가야 하므로
      // 여기서 던지지 않는다 — 플랫폼 층위 하나 때문에 인기 차트가 죽으면 안 된다.
      const [{ points, label }, platforms] = await Promise.all([
        sparkSeries(sql, rows.map(row => row.appid), { days: 7 }),
        platformsForApps(sql, rows.map(row => row.appid)).catch(() => new Map())
      ]);
      payload = {
        ...chartPayload(rows.map(row => ({
          ...row,
          spark: points.get(row.appid) ?? null,
          platforms: platforms.get(row.appid) ?? []
        }))),
        sparkLabel: label
      };
