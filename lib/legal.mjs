...
<article class="doc">
  ...
  ${section('한 줄 요약', [
    config.mail
      ? '이 사이트는 <strong>회원가입이 없습니다.</strong> 이메일 알림을 직접 신청하신 경우에만 <strong>이메일 주소 하나</strong>를 보관하며, 그 밖의 개인정보는 수집하지 않습니다. 위시리스트는 서버로 전송되지 않고 사용자의 브라우저 안에만 저장됩니다.'
  ...
  ${s('수집하지 않는 것', [
    ...
    list([
      config.mail ? '이름, 전화번호, 주소 — 이메일 알림에도 주소 외의 정보는 받지 않습니다' : '이름, 이메일 주소, 전화번호, 주소',
  ...
  ${s('이용자의 권리', [
    config.mail
      ? `이메일 알림을 신청하지 않으셨다면 이 사이트가 보관하는 개인정보가 없습니다. 신청하신 경우, 메일의 수신 거부 링크를 누르는 것만으로 <strong>즉시 발송이 중단되고 알림 설정이 삭제</strong>되며 남은 기록도 30일 뒤 사라집니다. 그 전에 즉시 삭제를 원하시거나 보관 내용을 확인하고 싶으시면 ${contactLine()}로 연락해 주세요.`
  ...
  return doc(layout({
    ...
    description: config.mail
      ? `${SITE_NAME}의 개인정보처리방침. 회원가입이 없고, 직접 신청하신 이메일 알림 외에는 개인정보를 수집하지 않습니다.`
      : `${SITE_NAME}는 회원가입과 개인정보 수집이 없습니다. 위시리스트는 브라우저에만 저장되며 서버로 전송되지 않습니다.`,
    path: '/privacy',
