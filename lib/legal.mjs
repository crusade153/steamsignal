...
export const POLICY_UPDATED = '2026-09-06';
...
// 방침의 절 번호는 손으로 붙이지 않는다. 이메일 항목이 설정에 따라 있다 없다 하므로
// 번호를 박아 두면 메일이 꺼진 사이트에서 3번이 사라진 방침이 된다.
const numbered = () => {
  let n = 0;
  return (title, paragraphs) => section(`${++n}. ${title}`, paragraphs);
};
...
<article class="doc">
  ...
  ${section('한 줄 요약', [
    '이 사이트는 <strong>계정 없이 전부 이용할 수 있습니다.</strong> 순위·급상승·할인·게임 상세·위시리스트를 보는 데 로그인이 필요하지 않고, 그렇게 이용하시면 이 사이트가 보관하는 개인정보가 없습니다.',
    config.mail
      ? '직접 <strong>계정을 만들거나 이메일 알림을 신청하신 경우에만</strong> 이메일 주소를 보관합니다. 위시리스트는 기본적으로 브라우저 안에만 저장되며, 로그인하신 경우에만 계정에도 함께 저장됩니다.'
      : '직접 <strong>계정을 만드신 경우에만</strong> 이메일 주소를 보관합니다. 위시리스트는 기본적으로 브라우저 안에만 저장되며, 로그인하신 경우에만 계정에도 함께 저장됩니다.'
  ])}

  ${s('수집하지 않는 것', [
    '계정을 만드셔도 다음은 수집하거나 저장하지 않습니다.',
    list([
      '이름, 전화번호, 주소 — 계정에도 이메일 주소 외의 정보는 받지 않습니다',
      '<strong>비밀번호 원문</strong> — 되돌릴 수 없는 해시(scrypt)만 저장하며, 운영자도 원래 비밀번호를 알 수 없습니다',
      '<strong>접속 IP 와 브라우저 정보</strong> — 로그인 기록에도 남기지 않습니다',
  ...
  ${s('계정을 만든 경우', [
    '계정은 <strong>선택 사항</strong>입니다. 만드시면 담아 둔 게임을 다른 기기에서도 볼 수 있고, 만들지 않으셔도 사이트의 모든 화면을 그대로 쓰실 수 있습니다.',
    list([
      '<strong>수집 항목</strong> — 이메일 주소, 비밀번호 해시, 담아 두신 게임의 Steam 앱 번호 목록, 가입·마지막 로그인 시각.',
      '<strong>이용 목적</strong> — 로그인 확인과, 담아 둔 게임을 기기 사이에서 이어 주는 것. 이 목적 외에는 사용하지 않으며 광고에 활용하지 않습니다.',
      '<strong>로그인 상태 유지</strong> — 브라우저에 무작위 값이 담긴 쿠키(<code>sp_session</code>) 하나가 저장되고, 서버에는 그 값의 해시만 남습니다. 30일 뒤 만료되며 로그아웃하면 즉시 삭제됩니다. 이 쿠키는 로그인 유지에만 쓰이고 추적에 쓰이지 않습니다.',
      '<strong>보관 기간</strong> — 계정을 삭제하실 때까지 보관합니다. 만료된 로그인 세션은 자동으로 삭제됩니다.',
      '<strong>파기 방법</strong> — <a href="/account">계정 화면</a>에서 직접 삭제하실 수 있고, 누르는 즉시 계정·세션·저장된 목록이 데이터베이스에서 삭제됩니다. 별도의 사본이나 백업 보관본을 만들지 않습니다.'
    ]),
    '<strong>제3자 제공</strong> — 계정 정보는 어디에도 제공하지 않습니다.'
  ])}

  ${config.mail ? s('이메일 알림을 신청한 경우', [
    '가격 알림이나 주간 리포트를 신청하면 <strong>이메일 주소</strong>가 이 사이트의 데이터베이스에 저장됩니다. 신청하지 않으면 아무것도 저장되지 않습니다.',
    list([
      '<strong>수집 항목</strong> — 이메일 주소, 알림을 신청한 게임과 목표 가격, 신청·확인·발송 시각. <strong>IP 주소와 접속 기기 정보는 저장하지 않습니다.</strong>',
      '<strong>이용 목적</strong> — 신청하신 알림 메일의 발송, 그리고 중복 발송을 막기 위한 발송 기록 관리. 이 목적 외에는 사용하지 않습니다.',
      '<strong>동의 방식</strong> — 확인 메일의 버튼을 누르기 전에는 어떤 메일도 보내지 않습니다(더블 옵트인). 신청한 적 없는 주소로는 확인 메일 한 통 외에 아무것도 가지 않습니다.',
      '<strong>보관 기간</strong> — 확인하지 않은 신청은 <strong>30일</strong> 뒤, 수신 거부한 주소는 <strong>30일</strong> 뒤 자동으로 삭제됩니다. 발송 기록은 <strong>90일</strong> 뒤 삭제됩니다. 구독 중인 주소는 수신을 거부하실 때까지 보관합니다.',
      '<strong>파기 방법</strong> — 데이터베이스에서 해당 행을 삭제하며, 별도의 사본이나 백업 보관본을 만들지 않습니다.'
    ]),
    '<strong>처리 위탁</strong> — 메일 발송은 Resend, Inc. 에 위탁합니다. 발송에 필요한 이메일 주소와 메일 본문이 이 업체로 전달되며, 처리 방식은 <a href="https://resend.com/legal/privacy-policy" target="_blank" rel="noopener noreferrer nofollow">Resend 개인정보처리방침</a>을 따릅니다. 이 밖의 제3자에게는 제공하지 않습니다.',
    '<strong>수신 거부</strong> — 모든 메일 맨 아래의 링크로 언제든 끊을 수 있고, 별도의 확인 절차나 로그인이 필요 없습니다. 거부하시면 담아 두신 알림 설정도 함께 삭제됩니다. <a href="/alerts">알림 안내 페이지</a>에서도 다시 신청하거나 해지하실 수 있습니다.'
  ]) : ''}

  ${s('위시리스트가 저장되는 곳', [
    '<strong>로그인하지 않은 경우</strong> — 담은 게임 목록은 브라우저의 <code>localStorage</code>에만 저장됩니다. 이 값은 <strong>서버로 전송되지 않고</strong>, 다른 기기나 다른 브라우저에서도 보이지 않으며, 운영자도 볼 수 없습니다.',
    '<strong>로그인한 경우</strong> — 같은 목록이 계정에도 함께 저장되어 다른 기기에서 이어집니다. 저장되는 것은 게임의 Steam 앱 번호뿐입니다. 로그아웃하면 서버로 더 보내지 않으며, 브라우저에 있던 목록은 그대로 남습니다.',
    '브라우저의 사이트 데이터를 지우거나 위시리스트 페이지에서 항목을 삭제하면 즉시 사라집니다. 계정에 저장된 목록은 계정을 삭제하면 함께 사라집니다. 별도의 요청 절차가 필요 없습니다.'
  ])}
  ...
  ${s('이 사이트가 저장하는 데이터', [
    'Steam 공개 API 에서 받은 게임 제목·장르·가격·리뷰 수와, 10분 간격으로 기록한 동시접속자 수치가 들어 있습니다.',
    config.mail
      ? '방문자에 관한 정보는 <strong>직접 신청하신 이메일 주소를 제외하면</strong> 들어 있지 않습니다. 접속 기록·검색어·열람한 페이지는 데이터베이스에 남지 않습니다.'
      : '방문자에 관한 정보는 들어 있지 않습니다.'
  ])}
  ...
  ${s('이용자의 권리', [
    config.mail
      ? `계정을 만들지 않으셨고 이메일 알림도 신청하지 않으셨다면 이 사이트가 보관하는 개인정보가 없습니다. 계정은 <a href="/account">계정 화면</a>에서 직접 삭제하실 수 있고, 누르는 즉시 사라집니다. 이메일 알림은 메일 맨 아래의 수신 거부 링크로 <strong>즉시 중단</strong>되며 남은 기록도 30일 뒤 사라집니다. 그 밖에 보관 내용을 확인하고 싶으시면 ${contactLine()}로 연락해 주세요.`
      : `계정을 만들지 않으셨다면 이 사이트가 보관하는 개인정보가 없습니다. 계정은 <a href="/account">계정 화면</a>에서 직접 삭제하실 수 있고, 누르는 즉시 사라집니다. 그 밖에 문의할 내용이 있으면 ${contactLine()}로 연락해 주세요.`,
    '만 14세 미만 아동을 대상으로 하지 않으며, 아동의 개인정보를 의도적으로 수집하지 않습니다.'
  ])}

  ${s('방침의 변경', [
    '이 방침이 바뀌면 이 페이지의 최종 개정일을 함께 갱신합니다. 수집 항목이 늘어나는 변경은 시행 전에 이 페이지에 먼저 반영합니다.'
  ])}
</article>`;

  return doc(layout({
    title: `개인정보처리방침 | ${SITE_NAME}`,
    description: `${SITE_NAME}의 개인정보처리방침. 계정 없이 전부 이용할 수 있으며, 직접 계정을 만들거나 알림을 신청하신 경우에만 이메일 주소를 보관합니다.`,
    path: '/privacy',
...
<article class="doc">
  ...
  ${section('1. 서비스의 성격', [
    `${SITE_NAME}는 Steam 공개 데이터를 수집해 동시접속자·평가·가격의 추이를 보여 주는 무료 정보 사이트입니다. 아무것도 판매하지 않으며, <strong>계정 없이 전부 이용할 수 있습니다</strong> — 계정은 담아 둔 게임을 기기 사이에서 이어 주는 선택 기능입니다.`,
  ...
  ${section('2. 데이터의 정확성과 한계', [
    ...
    list([
      ...
      ...(config.mail ? ['<strong>알림 메일</strong>은 최선의 노력으로 보내며 <strong>도달·적시성을 보증하지 않습니다.</strong> 수집 주기와 발송 주기 때문에 할인이 시작된 뒤 메일이 도착하기까지 시간이 걸릴 수 있고, 짧은 할인은 놓칠 수 있습니다. 메일을 받지 못한 것을 이유로 한 손해는 배상하지 않습니다.'] : [])
    ]),
