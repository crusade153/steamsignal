    ...
    const games = await Promise.all(ids.map(async appid => {
      const [detail, review] = await Promise.all([
        cached(`detail:${appid}`, 30 * MINUTE, async () => {
          const data = (await request(`https://store.steampowered.com/api/appdetails?appids=${appid}&cc=kr&l=koreana`))[appid];
          if (!data?.success || !data.data) throw new Error('Store details unavailable');
          return data.data;
        }).catch(() => null),
        cached(`review:${appid}`, 10 * MINUTE, async () => {
          const data = await request(`https://store.steampowered.com/appreviews/${appid}?json=1&language=all&num_per_page=1&filter=all&purchase_type=all`);
