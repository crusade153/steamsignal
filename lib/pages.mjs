...
<article class="game-detail">
  ...
  ${rows.length ? listSection('일자별 신규 리뷰', '최신순 · 최대 30구간', table(
    ['<th scope="col">기간</th>', '<th scope="col" class="numeric">신규 리뷰</th>',
      '<th scope="col" class="numeric">긍정</th>', '<th scope="col" class="numeric">부정</th>',
      '<th scope="col" class="numeric">그 구간 긍정률</th>'],
    rows.map(row => `<tr>
      <td>${esc(formatDay(row.day))}${row.from !== row.day ? `<span class="cell-sub">${esc(formatDay(row.from))} 이후</span>` : ''}</td>
      <td class="numeric"><span class="player-number">${esc(num(row.total))}</span></td>
      <td class="numeric peak-number">${esc(num(row.positive))}</td>
      <td class="numeric peak-number">${esc(num(row.negative))}</td>
      <td class="numeric">${isNum(row.ratio) ? `<span class="review-score ${row.ratio >= 75 ? '' : row.ratio >= 50 ? 'mixed' : 'negative'}">${row.ratio}%</span>` : '<span class="missing">—</span>'}</td>
    </tr>`)
...
async function chartsPage(sql, view) {
  const rows = await q.weekly(sql, { days: view.days, limit: 50 });
  ...
  const body = rows.length
    ? table(
      ['<th scope="col" class="rank-column">순위</th>', '<th scope="col" class="game-column">게임</th>',
        `<th scope="col" class="numeric">${esc(view.word)} 평균 동접</th>`, '<th scope="col" class="numeric">기간 내 최고</th>',
...
export async function dealsLowPage(sql) {
  ...
  const body = rows.length
    ? table(
      ['<th scope="col" class="game-column">게임</th>', '<th scope="col" class="numeric">할인</th>',
        '<th scope="col" class="numeric price-column">현재가 = 최저가</th>', '<th scope="col" class="numeric">정가</th>',
        '<th scope="col" class="numeric">Steam 평가</th>', '<th scope="col" class="numeric">관측</th>'],
      rows.map((row, i) => `<tr>
        <td>${gameCell(row, { eager: i < 3 })}</td>
        <td class="numeric"><span class="discount">-${row.discount_percent}%</span></td>
        <td class="numeric price-column">${priceCell(row)}<span class="cell-sub badge-low">역대 최저가</span></td>
        <td class="numeric">${isNum(row.initial_price) ? `<span class="original-price">${esc(won(row.initial_price))}</span>` : '<span class="missing">—</span>'}</td>
        <td class="numeric">${reviewCell(row)}</td>
        <td class="numeric peak-number">${row.observations}회<span class="cell-sub">${esc(formatMoment(row.first_seen)?.slice(0, 12) || '')}부터</span></td>
      </tr>`)
...
const listingTable = (rows, { showRelease = false } = {}) => table(
  ['<th scope="col" class="rank-column">#</th>', '<th scope="col" class="game-column">게임</th>',
    showRelease ? '<th scope="col" class="numeric">발매일</th>' : '<th scope="col" class="numeric">현재 동접</th>',
    '<th scope="col" class="numeric">Steam 평가</th>', '<th scope="col" class="numeric">메타크리틱</th>',
    '<th scope="col" class="numeric price-column">가격</th>'],
  rows.map((row, i) => `<tr>
    <td class="rank-cell${i < 3 ? ' top' : ''}">${i + 1}</td>
    <td>${gameCell(row, { eager: i < 3 })}</td>
    <td class="numeric">${showRelease
      ? (row.release_date ? esc(formatDay(row.release_date)) : '<span class="missing">미상</span>')
      : (isNum(row.players) ? `<span class="player-number">${esc(num(row.players))}</span>` : '<span class="missing">차트 밖</span>')}</td>
    <td class="numeric">${reviewCell(row)}</td>
    <td class="numeric">${metaScoreCell(row.metacritic_score, null)}</td>
    <td class="numeric price-column">${priceCell(row)}${isNum(row.initial_price) && isNum(row.final_price) && row.initial_price > row.final_price ? `<span class="cell-sub original-price">${esc(won(row.initial_price))}</span>` : ''}</td>
  </tr>`)
);
...
export async function genrePage(sql, { genre }) {
  ...
  const rows = await q.genreGames(sql, name, GENRE_LIMIT);
  ...
  return html(layout({
    ...
    body: `<section class="page-intro"><div class="eyebrow">GENRE</div><h1>${esc(name)}</h1><p>${esc(name)} 장르 ${rows.length}개 게임을 현재 동시접속자 순으로 정렬했습니다.${players.length ? ` 지금 이 장르에서 ${esc(num(players.reduce((sum, row) => sum + row.players, 0)))}명이 플레이 중입니다.` : ''}</p>${comboLinks(name, rows)}</section>${listingTable(rows)}`
  }));
}
...
async function genreListing(sql, genre, view) {
  ...
  const rows = await view.fetch(sql, name);
  if (rows.length < MIN_COMBO_GAMES) return notFound(view.thin(name));
  ...
  return html(layout({
    ...
    body: `<section class="page-intro"><div class="eyebrow">${esc(view.eyebrow)}</div><h1>${esc(view.heading(name))}</h1>` +
      ...
      listingTable(rows) + `<p class="muted table-footnote">${view.footnote}</p>`
  }));
}
...
export async function releasePage(sql, { year }) {
  ...
  const rows = await q.releaseYearGames(sql, parsed, 60);
  if (!rows.length) return notFound(`${parsed}년에 발매된 게임이 아직 목록에 없습니다.`);
  ...
  return html(layout({
    ...
    body: `<section class="page-intro"><div class="eyebrow">RELEASED ${parsed}</div><h1>${parsed}년에 나온 게임</h1>` +
      ...
      listingTable(rows, { showRelease: true }) +
