...
import {
  ...
  metaScoreCell, lineChart, emptyState, siteOrigin
} from './render.mjs';
...
<article class="game-detail">
  <header class="hero">
    ...
    <div class="hero-text">
      ...
      ${genreLinks}
      <p class="hero-actions"><a class="button primary" href="${esc(steamStoreUrl(app.appid))}" target="_blank" rel="noopener noreferrer">Steam 스토어에서 보기 ↗</a></p>
    </div>
  </header>
...
export async function dealsPage(sql) {
  ...
  const body = rows.length
    ...
    ) + '<p class="muted table-footnote">Steam 긍정률 75% 이상인 할인만 싣습니다. 역대 최저가 판정은 Steam Pulse 가 기록을 시작한 이후의 최저가 기준입니다. 가격은 한국 스토어 기준이며 구매 전 실제 가격을 확인하세요.</p>'
