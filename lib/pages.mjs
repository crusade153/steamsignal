...
import {
  ...
  gamePath, genrePath, steamStoreUrl, safeImage, gameCell, reviewCell, priceCell, discountDeadlineCell,
  metaScoreCell, lineChart, emptyState, siteOrigin, adSlot, dataTable, COL
} from './render.mjs';
import { LEGAL_HANDLERS, staticDocPaths } from './legal.mjs';
// 보관 기간과 저장소 예산은 환경변수라 화면에 못 박을 수 없다. 실제로 적용 중인 값을 그대로 적는다.
import { RETENTION, STORAGE_BUDGET_BYTES } from './db.mjs';
import { PLATFORMS } from './platforms.mjs';
import { CONSOLES } from './render.mjs';
...
// 추적군 비중·전일 동시간·일평균 전일 대비는 서로 다른 질문이므로 계산 기준도 함께 돌려준다.
...
// 목록 행에 콘솔 출시를 붙인다. 질의 하나로 묶어 받는다(withSparks 와 같은 방식).
//
// **못 받아도 목록은 그대로 뜬다.** 이 층위는 Steam 수집과 분리돼 있어서, 여기서 던지면
// 플랫폼 데이터 하나 때문에 인기 차트가 통째로 죽는다. 그 거래는 하지 않는다.
async function withPlatforms(sql, rows) {
  if (!rows.length) return rows;
  try {
    const byApp = await q.platformsForApps(sql, rows.map(row => row.appid));
    return rows.map(row => ({ ...row, platforms: byApp.get(row.appid) ?? [] }));
  } catch {
    return rows;
  }
}
...
// 다른 플랫폼 출시. **콘솔 동접·가격을 그리는 자리가 아니다** — 세 콘솔은 그것을 공개하지 않는다.
// 여기 있는 것은 공개된 사실뿐이고, 우리가 아는 만큼만 적는다.
//
// 세 가지 상태를 구분해서 쓴다. 날짜가 있으면 날짜를, 플랫폼만 확인됐으면 `출시일 미상`을,
// 그 플랫폼에 없으면 `없음`을 적는다. 셋을 뭉뚱그리면 "정보가 없다"와 "그 기계엔 안 나왔다"가
// 같은 칸이 되는데, 그 둘은 사용자에게 정반대의 뜻이다.
//
// 아무 정보도 없으면 절 자체를 그리지 않는다 — 빈 칸 셋은 없느니만 못하다(CLAUDE.md 규칙 11).
export function platformSection(releases) {
  if (!releases?.length) return '';
  const byPlatform = new Map(releases.map(row => [row.platform, row]));
  const dated = releases.filter(row => row.released_on).length;

  const items = PLATFORMS.map(platform => {
    const row = byPlatform.get(platform.key);
    const value = !row
      ? '<em class="missing">없음</em>'
      : row.released_on
        ? `${esc(formatDay(row.released_on))} 출시`
        : '<em class="missing">출시일 미상</em>';
    return `<li><span>${esc(platform.name)}</span><strong>${value}</strong></li>`;
  }).join('');

  const note = dated ? `출시일 확인 ${dated}건` : '플랫폼만 확인';
  return listSection('다른 플랫폼', note, `<ul class="fact-list">${items}</ul>
    <p class="muted">출시 여부와 날짜는 Wikidata(CC0)에서 가져온 공개 정보이며, Steam appid 로 직접 대조한 항목만 씁니다. 제목이 비슷하다는 이유로 잇지 않습니다. <strong>콘솔의 동시접속자와 가격은 제공하지 않습니다</strong> — 세 플랫폼 모두 그 값을 공개하지 않아, 추정치를 숫자처럼 보여 주지 않기 위해서입니다.</p>`);
}
...
export async function gamePage(sql, { slug }) {
  ...
  const [hourly, daily, peak, low, trend, related, releases] = await Promise.all([
  ...
  if (!isNum(app.final_price)) {
    ...
    priceBlock = `<p class="price-headline">${app.discount_percent > 0 ? `<span class="discount">-${app.discount_percent}%</span>` : ''}${esc(now)}` +
      ...
      `<ul class="fact-list">` +
      (app.discount_percent > 0 ? `<li><span>할인 종료</span><strong>${discountDeadlineCell(app)}</strong></li>` : '') +
  ...
  const body = rows.length
    ? dataTable(
      [
        ...
        COL.price(),
        { label: '할인 종료', cellClass: 'deadline-cell', cell: row => discountDeadlineCell(row) },
        COL.review(),
        COL.meta(),
        COL.players()
      ],
      rows
    ) + '<p class="muted table-footnote">할인 종료일은 Steam 한국시간 상점이 표시한 날짜입니다. Steam 긍정률이 75% 미만인 할인은 제외했습니다. 아직 평가를 수집하지 못한 게임은 <b>집계 전</b>으로 표시되며 목록 뒤쪽에 놓입니다 — 평가가 나쁘다는 뜻이 아닙니다. 역대 최저가 판정은 Steam Pulse 가 기록을 시작한 이후의 최저가 기준이라, 그 이전에 더 쌌던 적이 있을 수 있습니다. 가격은 한국 스토어 기준이며 구매 전 실제 가격을 확인하세요.</p>'
...
export async function dealsLowPage(sql) {
  ...
  const body = rows.length
    ? dataTable(
      ...
      rows
    ) + '<p class="muted table-footnote">할인 종료일은 Steam 한국시간 상점이 표시한 날짜이며, 정확한 종료 시각을 제공하지 않는 경우 날짜까지만 표시합니다. 여기서 <b>역대</b>는 Steam Pulse 가 기록을 시작한 이후입니다. 그 이전에 더 쌌던 적이 있을 수 있습니다. 가격 변동을 <b>두 번 이상</b> 관측한 게임만 싣습니다 — 한 번밖에 못 본 가격은 자동으로 최저가가 되어 버려 아무것도 알려 주지 못하기 때문입니다. 가격은 한국 스토어 기준이며 구매 전 실제 가격을 확인하세요.</p>'
  ...
  return html(layout({
    ...
    body: `<section class="page-intro"><div class="eyebrow">ALL-TIME LOW</div><h1>지금이 역대 최저가</h1><p>할인율 숫자는 정가를 어떻게 매겼느냐에 따라 얼마든지 커집니다. 이 목록은 다릅니다 — <strong>우리가 직접 기록한 가격 이력</strong>에서 지금 값이 가장 낮은 게임만 골랐고, <strong>한국시간 할인 종료일</strong>도 함께 보여 줍니다.</p><p class="page-links"><a class="button" href="/deals">할인 전체 보기</a></p></section>${body}`
  }));
}
...
const listingTable = (rows, { showRelease = false, sparkLabel = '전일 대비' } = {}) => dataTable(
  [
    COL.rank(),
    COL.game(),
    showRelease
...
// =============================================================================
// 콘솔 출시 — /platform · /platform/:key
//
// **여기서 콘솔의 동접·가격을 그리지 않는다.** 세 플랫폼 모두 그 값을 공개하지 않는다.
// 이 페이지가 답하는 질문은 하나다 — "지금 PC 에서 뜨는 이 게임, 내 콘솔에도 있나."
// 그래서 열은 Steam 동접(우리가 기록한 값)이고, 콘솔에서 오는 것은 출시 여부와 날짜뿐이다.
// =============================================================================

const PLATFORM_VIEWS = new Map(PLATFORMS.map(item => [item.key, item]));

// 플랫폼 목록의 표. listingTable 과 달리 '이 플랫폼 출시일' 열이 들어간다 —
// 이 페이지에서는 그게 정렬 근거이자 페이지의 이유다.
const platformTable = (rows, sparkLabel) => dataTable(
  [
    COL.rank(),
    COL.game(),
    COL.players(),
    {
      ...
      headClass: 'numeric',
      cellClass: 'numeric',
      // 날짜가 없는 것을 빈 칸으로 두지 않는다. '플랫폼은 확실한데 날짜 미상'이다.
      cell: row => (row.platform_released_on
        ? esc(formatDay(row.platform_released_on))
        : '<span class="missing">출시일 미상</span>')
    },
    COL.spark(sparkLabel),
    COL.review(),
    COL.price()
...
export async function platformsPage(sql) {
  const counts = await q.platformCounts(sql);
  const byKey = new Map(counts.map(row => [row.platform, row]));

  const cards = PLATFORMS.map(item => {
    const row = byKey.get(item.key);
    const games = row?.games ?? 0;
    // 링크는 **그 페이지가 실제로 열릴 때만** 건다. 판정 기준이 페이지·사이트맵과 같은 상수다.
    const open = games >= MIN_COMBO_GAMES;
    const inner = `<strong>${esc(item.name)}</strong><small>${games ? `${esc(num(games))}개 · 출시일 확인 ${esc(num(row.dated))}건` : '아직 확인된 게임이 없습니다'}</small>`;
    return open
      ? `<a class="mini-card" href="/platform/${item.key}">${inner}</a>`
      : `<span class="mini-card is-muted">${inner}</span>`;
  }).join('');

  const total = counts.reduce((sum, row) => sum + row.games, 0);
  const body = `
<section class="page-intro">
  <div class="eyebrow">CONSOLE RELEASES</div>
  <h1>콘솔 출시</h1>
  <p>Steam Pulse 가 추적하는 게임이 <strong>Xbox · PlayStation · Nintendo Switch</strong> 에도 나왔는지 정리했습니다. 지금까지 확인된 콘솔 출시는 ${esc(num(total))}건입니다.</p>
  <p class="page-links"><a class="button" href="/">인기 차트</a> <a class="button" href="/rising">급상승</a></p>
</section>

${listSection('플랫폼', '', `<div class="card-grid">${cards}</div>`)}

<section class="panel source-note">
  <h2>여기에 없는 것</h2>
  <p class="muted"><strong>콘솔의 동시접속자와 가격은 제공하지 않습니다.</strong> Xbox·PlayStation·Nintendo 는 전체 카탈로그의 실시간 동접이나 가격을 공개하는 공개 API 를 두고 있지 않습니다. 없는 숫자를 추정해 채우면 그 순간 이 사이트는 다른 미러와 같아지므로, 확인할 수 있는 사실(출시 여부와 날짜)만 적습니다.</p>
  <p class="muted">출시 정보는 <strong>Wikidata(CC0)</strong> 에서 가져오며, Steam 의 appid 로 직접 대조한 항목만 씁니다. 제목이 비슷하다는 이유로 잇지 않고, 한 게임에 항목이 둘 이상 걸리면 아예 쓰지 않습니다. 현재 매칭 상태는 <a href="/status">수집 상태</a>에 그대로 공개합니다.</p>
  <p class="muted">세대는 묶어서 셉니다 — PS4 와 PS5 는 둘 다 '플레이스테이션'입니다. 다만 <strong>PS3·Vita·Xbox 360·Wii U 같은 이전 세대는 넣지 않습니다</strong>. 그 기계에만 나온 게임을 '플레이스테이션 출시'라고 적으면 지금 콘솔을 든 사람에게 틀린 말이 됩니다.</p>
</section>`;

  return html(layout({
    title: `콘솔 출시 — Xbox · PlayStation · Switch | ${SITE_NAME}`,
    description: 'Steam 에서 인기 있는 게임이 Xbox·PlayStation·Nintendo Switch 에도 나왔는지, 언제 나왔는지 확인합니다.',
    path: '/platform',
    active: '/platform',
    breadcrumb: [{ label: '인기 차트', href: '/' }, { label: '콘솔 출시' }],
    body
  }), { cache: CACHE_LIST });
}

export async function platformPage(sql, { key }) {
  const view = PLATFORM_VIEWS.get(String(key));
  if (!view) return notFound('플랫폼을 찾을 수 없습니다.');

  const found = await q.gamesByPlatform(sql, view.key, 100);
  // 얇은 페이지를 만들지 않는다. 링크·사이트맵이 같은 상수를 본다(CLAUDE.md 규칙 15).
  if (found.length < MIN_COMBO_GAMES) {
    return notFound(`${view.name}에서 확인된 게임이 아직 충분하지 않습니다.`);
  }
  const { rows, sparkLabel } = await withSparks(sql, found);

  const dated = rows.filter(row => row.platform_released_on).length;
  const today = new Date().toISOString().slice(0, 10);
  const upcoming = rows.filter(row => row.platform_released_on && row.platform_released_on > today);

  const body = `
<section class="page-intro">
  <div class="eyebrow">CONSOLE RELEASES</div>
  <h1>${esc(view.name)} 출시</h1>
  <p>Steam Pulse 가 추적하는 게임 중 ${esc(view.name)}에도 나온 ${esc(num(rows.length))}개입니다. 정렬은 <strong>Steam 동시접속자</strong> 순입니다 — 콘솔의 동접은 공개되지 않아 제공하지 않습니다.</p>
  <p class="page-links">${PLATFORMS.filter(item => item.key !== view.key).map(item => `<a class="button" href="/platform/${item.key}">${esc(item.name)}</a>`).join(' ')} <a class="button" href="/platform">전체</a></p>
</section>

${upcoming.length ? listSection('출시 예정', `${upcoming.length}개`, `<ul class="fact-list">${upcoming
    .sort((a, b) => a.platform_released_on.localeCompare(b.platform_released_on))
    .map(row => `<li><span><a href="${esc(gamePath(row.appid, row.slug))}">${esc(row.title)}</a></span><strong>${esc(formatDay(row.platform_released_on))}</strong></li>`)
    .join('')}</ul>`) : ''}

${listSection('전체', `출시일 확인 ${dated}건 · 나머지는 날짜 미상`, platformTable(rows, sparkLabel))}

<section class="panel source-note">
  <h2>이 표를 어떻게 읽나</h2>
  <p class="muted"><strong>동시접속자는 Steam 의 값입니다.</strong> ${esc(view.name)}의 동접이 아닙니다 — 그 값은 공개되지 않습니다. 이 표는 "PC 에서 이만큼 플레이되는 게임이 ${esc(view.name)}에도 있다"를 보여 줍니다.</p>
  <p class="muted"><strong>출시일 미상</strong>은 그 기계에 안 나왔다는 뜻이 아니라, 출시는 확인됐는데 날짜를 확인하지 못했다는 뜻입니다. Wikidata 에 연도까지만 적힌 경우도 여기에 들어갑니다 — 없는 날짜를 1월 1일로 지어내지 않습니다.</p>
</section>`;

  return html(layout({
    title: `${view.name} 출시 게임 — Steam 인기작 ${rows.length}개 | ${SITE_NAME}`,
    description: `Steam 에서 인기 있는 게임 중 ${view.name}에도 출시된 ${rows.length}개를 동시접속자 순으로 정리했습니다.`,
    path: `/platform/${view.key}`,
    active: '/platform',
    breadcrumb: [{ label: '인기 차트', href: '/' }, { label: '콘솔 출시', href: '/platform' }, { label: view.name }],
    body
  }), { cache: CACHE_LIST });
}
...
// Game Pass — /gamepass
//
// **이 페이지의 값은 대부분 우리가 만든 것이다.** 마이크로소프트는 '지금 목록'만 공개하고
// "어제와 무엇이 달라졌나"는 어디에도 없다. 그래서 매일 목록을 찍어 두고 우리가 비교한다.
// 이 사이트가 동접에 하는 일과 정확히 같다.
//
// 넷을 구분해서 적는다. 섞으면 '누가 말한 것'과 '우리가 관찰한 것'이 한 덩어리가 된다.
//   입점 예정 — 공식 발표(Xbox Wire)를 읽은 것. 출처 링크를 함께 건다
//   곧 종료   — 공식 '곧 종료' 컬렉션이 준 것
//   입점·퇴점 — 우리가 어제와 비교해 만든 기록
//   기준선    — 관측 시작일. 그 전의 입점은 우리가 모른다
// =============================================================================

const gamepassCards = rows => `<div class="card-grid">${rows.map(row => `
  <div class="mini-card is-static">
    <span><strong>${esc(row.title)}</strong><small>${esc(row.developer || '개발사 미확인')}${row.on_console && row.on_pc ? ' · 콘솔 + PC' : row.on_pc ? ' · PC' : row.on_console ? ' · 콘솔' : ''}</small></span>
  </div>`).join('')}</div>`;

const gamepassEventList = rows => `<ul class="fact-list">${rows.map(row =>
  `<li><span>${esc(row.title)}</span><strong>${esc(formatDay(row.happened_on))}</strong></li>`).join('')}</ul>`;

export async function gamepassPage(sql) {
  const [[summary], leaving, upcoming, added, removed] = await Promise.all([
    q.gamepassSummary(sql),
    q.gamepassLeaving(sql, 40),
    q.gamepassUpcoming(sql, 40),
    q.gamepassEvents(sql, { event: 'added', limit: 24 }),
    q.gamepassEvents(sql, { event: 'removed', limit: 24 })
  ]);

  const present = summary?.present ?? 0;
  const since = formatDay(summary?.watching_since) || '오늘';

  // 수집이 한 번도 안 돌았으면 빈 표를 그리지 않고 그렇다고 적는다.
  if (!present) {
    return html(layout({
      title: `Game Pass 입퇴점 | ${SITE_NAME}`,
      description: 'Xbox Game Pass 에 어떤 게임이 들어오고 나가는지 매일 기록합니다.',
      path: '/gamepass',
      active: '/gamepass',
      breadcrumb: [{ label: '인기 차트', href: '/' }, { label: 'Game Pass' }],
      body: `<section class="page-intro"><div class="eyebrow">GAME PASS</div><h1>Game Pass 입퇴점</h1></section>
        ${emptyState('아직 카탈로그를 받지 못했습니다.', '하루 한 번 Game Pass 목록을 기록합니다. 첫 기록이 들어오면 여기에 나타납니다.')}`
    }), { cache: CACHE_MISS });
  }

  const sources = [...new Set(upcoming.map(row => row.source_url).filter(Boolean))].slice(0, 3);

  const body = `
<section class="page-intro">
  <div class="eyebrow"><span class="live-dot"></span> GAME PASS</div>
  <h1>Game Pass 입퇴점</h1>
  <p>지금 Game Pass 에 <strong>${esc(num(present))}개</strong>가 있습니다. 마이크로소프트는 '지금 목록'만 공개하기 때문에, <strong>무엇이 들어오고 나갔는지는 저희가 매일 목록을 기록해 직접 비교한 것</strong>입니다.</p>
  <p class="page-links"><a class="button" href="/platform/xbox">엑스박스 출시 게임</a> <a class="button" href="/platform">콘솔 출시</a></p>
</section>

<section class="metrics" aria-label="Game Pass 지표">
  ${metric('지금 카탈로그', `${esc(num(present))}개`, `콘솔 ${esc(num(summary.console_games))} · PC ${esc(num(summary.pc_games))}`)}
  ${metric('입점 예정', upcoming.length ? `${esc(num(upcoming.length))}개` : null, upcoming.length ? '공식 발표 기준' : '발표된 예정이 없습니다')}
  ${metric('곧 종료', summary.leaving ? `${esc(num(summary.leaving))}개` : null, summary.leaving ? '공식 목록 기준' : '지정된 게임이 없습니다')}
  ${metric('기록한 입퇴점', `${esc(num(summary.events))}건`, `${esc(since)}부터 관측`)}
</section>

${upcoming.length
    ? listSection('입점 예정', '공식 발표에서 읽었습니다', `<ul class="fact-list">${upcoming.map(row =>
      `<li><span>${esc(row.title)}${row.devices ? ` <small class="muted">${esc(row.devices)}</small>` : ''}</span><strong>${esc(formatDay(row.announced_for))}</strong></li>`).join('')}</ul>
  <p class="muted">Xbox Wire 의 공식 발표 글에서 읽은 값입니다. 게임 이름은 원문 그대로 둡니다 — 옮기다가 다른 게임이 되는 것을 막기 위해서입니다.${sources.length ? ` 출처: ${sources.map((url, i) => `<a href="${esc(url)}" target="_blank" rel="noopener noreferrer">발표 ${i + 1} ↗</a>`).join(' · ')}` : ''}</p>`)
    : listSection('입점 예정', '', emptyState('발표된 입점 예정이 없습니다.', '마이크로소프트가 공식 글로 발표하면 하루 안에 여기 나타납니다. 카탈로그 자체에는 예정 정보가 없어 발표에만 의존합니다.'))}

${leaving.length
    ? listSection('곧 종료', `${leaving.length}개 · 공식 목록`, `${gamepassCards(leaving)}
  <p class="muted">마이크로소프트가 '곧 종료'로 표시한 게임입니다. <strong>정확한 종료일은 공개되지 않습니다</strong> — 저희가 날짜를 추정해 적지 않는 이유입니다. 실제로 빠지면 아래 퇴점 기록에 그날로 남습니다.</p>`)
    : ''}

${added.length ? listSection('최근 입점', `${added.length}건 · 저희가 기록한 변화`, gamepassEventList(added)) : ''}

${removed.length ? listSection('최근 퇴점', `${removed.length}건 · 저희가 기록한 변화`, gamepassEventList(removed)) : ''}

${!added.length && !removed.length
    ? listSection('입점·퇴점 기록', '', emptyState('아직 비교할 어제가 없습니다.',
      `${esc(since)}에 첫 목록을 기록했습니다. 입점·퇴점은 어제와 오늘을 비교해야 나오므로 내일부터 쌓입니다.`))
    : ''}

<section class="panel source-note">
  <h2>이 숫자는 어디서 왔나</h2>
  <p class="muted"><strong>입점·퇴점은 저희가 만든 기록입니다.</strong> 마이크로소프트는 지금 카탈로그만 공개하고 변경 이력은 제공하지 않습니다. 그래서 하루 한 번 목록을 통째로 기록해 두고 전날과 비교합니다. <strong>${esc(since)} 이전의 입점은 저희가 알 수 없습니다</strong> — 그때 이미 있던 게임은 '입점'으로 세지 않습니다.</p>
  <p class="muted"><strong>입점 예정과 곧 종료는 마이크로소프트가 밝힌 값입니다.</strong> 다만 종료일은 공식적으로도 날짜가 나오지 않아 저희도 날짜를 적지 않습니다. 카탈로그에는 입점 예정 정보가 없어(미출시 등록이 1건뿐이고 나머지는 9998년 같은 자리표시자입니다) 예정은 공식 발표 글에서만 읽습니다.</p>
  <p class="muted">한국(KR) 지역·한국어 기준입니다. 지역마다 카탈로그가 다르므로 다른 나라에서는 목록이 다를 수 있습니다. 구독 요금과 게임 가격은 다루지 않습니다.</p>
</section>`;

  return html(layout({
    title: `Game Pass 입퇴점 — 들어온 게임과 빠진 게임 | ${SITE_NAME}`,
    description: `Xbox Game Pass 에 어떤 게임이 들어오고 나가는지 매일 기록합니다. 지금 ${present}개, 곧 종료 ${summary.leaving}개.`,
    path: '/gamepass',
    active: '/gamepass',
    breadcrumb: [{ label: '인기 차트', href: '/' }, { label: 'Game Pass' }],
    body
...
export async function statusPage(sql) {
  const [[status], jobs, gaps, [storage], [platforms]] = await Promise.all([
    q.collectionStatus(sql),
    q.jobStatus(sql),
    q.snapshotGaps(sql, { hours: 24, minutes: 20 }),
    q.storageUsage(sql),
    q.platformCoverage(sql)
  ]);
  ...
  // 저장소는 이 사이트의 진짜 한도다. 커버리지도 새 지표도 전부 여기서 막힌다 —
  // 그래서 남은 여유를 감추지 않고 적는다.
  const usedBytes = Number(storage?.total_bytes);
  const storageMb = Number.isFinite(usedBytes) ? Math.round(usedBytes / 1024 / 1024) : null;
  const budgetMb = Math.round(STORAGE_BUDGET_BYTES / 1024 / 1024);
  const storagePct = storageMb === null ? null : Math.round((usedBytes / STORAGE_BUDGET_BYTES) * 1000) / 10;
...
${platforms?.checked ? listSection('다른 플랫폼 매칭', `Wikidata · 마지막 조회 ${esc(formatMoment(platforms.last_checked) || '기록 없음')}`, `<ul class="fact-list">
  <li><span>Steam appid 로 항목을 확정한 게임</span><strong>${esc(num(platforms.matched))} / ${esc(num(platforms.tracked))}</strong></li>
  <li><span>Wikidata 에 항목이 없는 게임</span><strong>${esc(num(platforms.unmatched))}개</strong></li>
  <li><span>항목이 둘 이상이라 잇지 않은 게임</span><strong>${esc(num(platforms.ambiguous))}개</strong></li>
  <li><span>아직 조회하지 않은 게임</span><strong>${esc(num(Math.max(platforms.tracked - platforms.checked, 0)))}개</strong></li>
  <li><span>확인된 콘솔 출시 (그중 날짜까지)</span><strong>${esc(num(platforms.releases))}건 · ${esc(num(platforms.dated))}건</strong></li>
</ul>
<p class="muted">제목이 비슷하다는 이유로 잇지 않습니다. 한 appid 에 항목이 둘 이상 걸리면 자동으로 고르지 않고 검토 대기열로 보냅니다 — 잘못 이으면 다른 게임의 출시일이 표시됩니다.</p>`) : ''}

${listSection('보관 중인 기록', '', `<ul class="fact-list">
  <li><span>원시 동접 (10분 간격 · ${RETENTION.snapshotDays}일 보관)</span><strong>${esc(num(status?.snapshot_rows))}행</strong></li>
  <li><span>시간 롤업 (1시간 · ${RETENTION.hourlyDays}일 보관)</span><strong>${esc(num(status?.hourly_rows))}행</strong></li>
  <li><span>일 롤업 (하루 · 영구 보관)</span><strong>${esc(num(status?.daily_rows))}행</strong></li>
  <li><span>저장소 사용량 (예산 ${esc(num(budgetMb))}MB)</span><strong>${storageMb === null ? '측정하지 못했습니다' : `${esc(num(storageMb))}MB · ${storagePct}%`}</strong></li>
...
<section class="panel source-note">
  ...
  <p class="muted"><strong>저장소</strong>는 이 사이트가 기록을 얼마나 더 쌓을 수 있는지를 정합니다. 여유가 줄면 보관 기간이 자동으로 짧아지고(원시 ${RETENTION.snapshotDays}일 → 최소 3일), 그래도 부족하면 감시 잡이 실패해 운영자에게 알립니다. <strong>일 단위 기록은 어느 경우에도 지우지 않습니다</strong> — 그게 이 사이트의 영구 기록입니다.</p>
...
const STATIC_PATHS = [
  ...
  { path: '/platform', priority: '0.6', changefreq: 'weekly' },
  { path: '/gamepass', priority: '0.7', changefreq: 'daily' },
...
export async function sitemap(sql) {
  ...
  // 플랫폼 페이지도 **열리는 것만** 싣는다. 판정 기준은 페이지·링크와 같은 상수다 —
  // 사이트맵에 있는데 404 인 URL 은 색인 전체에 손해다(CLAUDE.md 규칙 15).
  const platformUrls = platformRows
    .filter(row => row.games >= MIN_COMBO_GAMES)
    .map(row => entry(`${origin}/platform/${row.platform}`, { changefreq: 'daily', priority: '0.6' }));
