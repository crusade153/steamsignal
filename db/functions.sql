...
-- ---------------------------------------------------------------------------
-- 보관정책. 기본은 원시 7일, 시간 롤업 90일. 일 롤업은 지우지 않는다.
-- 이 함수가 없으면 Neon 무료 0.5GB 를 1년 안에 넘긴다 — 파이프라인의 필수 부품이다.
--
-- **여기의 기본값은 대비책일 뿐이다.** 실제로는 lib/collect.mjs 의 prune 잡이 매번
-- 저장소 사용률을 재서 planRetention() 이 계산한 기간을 인자로 넘긴다. 기간을 바꾸려고
-- 이 파일을 고치면 마이그레이션을 다시 돌려야 하지만(CLAUDE.md 규칙 24),
-- 환경변수(RETENTION_SNAPSHOT_DAYS / RETENTION_HOURLY_DAYS / DB_SIZE_BUDGET_MB)로 바꾸면 배포만으로 끝난다.
--
-- p_snapshot_days 를 3 밑으로 내리지 말 것. rollup_player_daily(2) 가 이틀 치 원시를
-- 다시 읽기 때문에, 원시가 먼저 지워지면 온전한 하루가 조각으로 덮이고 그 조각이 영구 보관된다.
-- ---------------------------------------------------------------------------
